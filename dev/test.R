...
row_covariates <- cbind(1, matrix(rnorm(6 * npc, sd = 1), ncol = 2)) # cbind(
#     1,
#     c(rep(1, npc), rep(-1, npc), rep(0, npc)),
#     c(rep(-1, npc), rep(2, npc), rep(0, npc))
# ) + 
...
pZ1 <- softmax_rows(row_covariates %*% B)

Z1_factor <- sapply(seq_len(nrow(pZ1)), function(idx) {
    sample(x = seq_len(ncol(pZ1)), size = 1, prob = pZ1[idx, ])
})

Z1 <- t(sapply(seq_along(Z1_factor), function(idx) {
    vec <- rep(0, ncol(pZ1))
    vec[Z1_factor[idx]] <- 1
    vec
}))

pZ1_g <- softmax_rows(row_covariates %*% B_g)
Z1_factor_g <- sapply(seq_len(nrow(pZ1_g)), function(idx) {
    sample(x = seq_len(ncol(pZ1_g)), size = 1, prob = pZ1_g[idx, ])
})

Z2 <- round(softmax_rows(col_covariates %*% G))


Z1_factor <- as.factor(Z1_factor)
indata <- data.frame(Z1 = Z1_factor, row_covariates)
indata$Z1 <- relevel(indata$Z1, ref = paste(ncol(B)))

indata_ref_gauche <- data.frame(Z1 = Z1_factor, row_covariates)
indata_ref_gauche$Z1 <- relevel(indata_ref_gauche$Z1, ref = "1")
...
fitted_pZ1 <- multinom_fit_Z1$fitted.values[, c(2,3,1)]

multinom_fit_Z1_g <- multinom(Z1 ~ -1 + X1 + X2 + X3, data = indata_ref_gauche)
summary(multinom_fit_Z1_g)
...
microbenchmark("no opt" = {
fit_covariates <- dispatcher(membership_name = "SBM", model_name = "bernoulli", membership_init = list(Z=start_Z, nodes_covariates=row_covariates), network = list(adjacency = M), real_EM = TRUE)})
