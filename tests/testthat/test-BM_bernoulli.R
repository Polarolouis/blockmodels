...
test_that("BM_bernoulli SBM with nodes covariates runs and passes nodes covariates", {
    npc <- 10
    Q <- 2
    n <- npc * Q
    Z <- diag(Q) %x% matrix(1, npc, 1)
    ...
    expect_model_estimation(model)
    expect_model_nodes_covariates(model, X)
})

test_that("BM_bernoulli SBM_sym with nodes covariates runs and passes nodes covariates", {
    npc <- 10
    Q <- 2
    n <- npc * Q
    Z <- diag(Q) %x% matrix(1, npc, 1)
    ...
    model <- BM_bernoulli("SBM_sym", M, nodes_covariates = list(node = X), plotting = "", explore_min = 2, explore_max = 2, ncores = 2, verbosity = 0)
    expect_model_estimation(model)
    expect_model_nodes_covariates(model, X)
})

test_that("BM_bernoulli LBM with nodes covariates runs and passes nodes covariates", {
    npc <- c(20, 10)
    Q <- c(1, 2)
    n <- npc * Q
    Z1 <- diag(Q[1]) %x% matrix(1, npc[1], 1)
    Z2 <- diag(Q[2]) %x% matrix(1, npc[2], 1)
    X_row <- cbind(1, runif(n[1]), runif(n[1]))
    X_col <- cbind(1, runif(n[2]), runif(n[2]))
    P <- matrix(runif(Q[1] * Q[2]), Q[1], Q[2])
    M <- 1 * (matrix(runif(n[1] * n[2]), n[1], n[2]) < Z1 %*% P %*% t(Z2))

    model <- BM_bernoulli("LBM", M, nodes_covariates = list(row = X_row, col = X_col), plotting = "", explore_min = 2, explore_max = 2, ncores = 2, verbosity = 0)
    expect_model_estimation(model)
    expect_model_row_col_covariates(model = model, expected_row_covariates = X_row, expected_col_covariates = X_col)
})
...
test_that("BM_bernoulli SBM fails gracefully when there are too much NAs", {
    skip("Sufficient condition to be found.")
    set.seed(12)
